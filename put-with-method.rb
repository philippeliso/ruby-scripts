# ruby put-with-method.rb -e "Repos.new.new_method"

require 'uri'
require 'net/https'
require 'json'

ACCESS_TOKEN = "Bearer 1111111111111"  # svcacc_atlassian

class Repos

    ### Get all project keys
    projects_url = URI("https://bitbucket.intranet/rest/api/1.0/projects?limit=999")
    ...
    projects_request["authorization"] = ACCESS_TOKEN
    ...
    def update_project_group_names(project_key, project_group_name, project_permission)
        ...
        put_slug_groups_url = URI("https://bitbucket.intranet/rest/api/1.0/projects/#{project_key}/permissions/groups?name=#{project_group_name}&permission=#{project_permission}")
        ...
        put_slug_groups_request["authorization"] = 'Bearer 1111111111' # svcacc_atlassian
    ...
    def update_slug_group_names(project_key, repo_slug, slug_group_name, slug_group_permission)
        ...
        put_slug_groups_url = URI("https://bitbucket.intranet/rest/api/1.0/projects/#{project_key}/repos/#{repo_slug}/permissions/groups?name=#{slug_group_name}&permission=#{slug_group_permission}")
        puts put_slug_groups_url
    ...
    projects_keys.map do |pk|
        ...
        groups_request["authorization"] = 'Bearer 1111111111' # svcacc_atlassian
        ...
        groups_json["values"].each do |value|
            if value["group"]["name"].match(/^l-scrum-/)
                ...
                Repos.new.update_project_group_names(key, new_group, value["permission"])
...
### Get all repositories of all projects
        ...
        repos_request["authorization"] = 'Bearer 1111111111' # svcacc_atlassian
...
### Get all groupos permission of repositories of all projects
        ...
        slug_permissions = slugs_repo.map do |slug|
            ...
            slug_groups_request["authorization"] = 'Bearer 1111111111' # svcacc_atlassian
            ...
            slug_groups_json["values"].each do |value|
                if value["group"]["name"].match(/^l-scrum-/)
                    ...
                    Repos.new.update_slug_group_names(key, slug, new_slug_group_name, value["permission"])
                elsif !value["group"]["name"].match(/^[gG]_s_p_scrum-/) # primeira letra ficou minuscula, ???
                    puts "Grupos sem match #{value["group"]["name"]}"
                end 
            end
